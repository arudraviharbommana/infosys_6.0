...
def match_skills():
    ...
    try:
        ...
        # Calculate the match score and get detailed results
        match_results = job_matcher.calculate_match_score(resume_text, job_description)

        # Generate learning recommendations based on the gap
        resume_skills = set(resume_analysis['skills'].keys())
        job_skills = set(job_analysis['skills'].keys())
        learning_path = recommendation_engine.generate_learning_path(resume_skills, job_skills)
