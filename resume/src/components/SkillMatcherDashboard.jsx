...
const SkillMatcherDashboard = ({ user, onLogout }) => {
  ...
  // Handle analysis using new backend API
  const handleAnalysis = async () => {
    if (backendStatus !== 'connected') {
      setError('Backend is not connected. Please check the server.');
      return;
    }

    // For manual input, create a text file
    let fileToAnalyze = resumeFile;
    if (useManualInput && manualResumeText.trim()) {
      const blob = new Blob([manualResumeText], { type: 'text/plain' });
      fileToAnalyze = new File([blob], 'manual_resume.txt', { type: 'text/plain' });
    }

    if (!fileToAnalyze || !jobDescription.trim()) {
          ...
          <div className="action-buttons">
            <button
              ...
              disabled={
                isAnalyzing || 
                backendStatus !== 'connected' ||
                (!resumeFile && !manualResumeText.trim()) || 
                !jobDescription.trim()
              }
            ...
            {backendStatus === 'disconnected' && (
              <button onClick={checkBackendHealth} className="connect-btn">
                🔄 Reconnect Backend
              </button>
            )}
          </div>
